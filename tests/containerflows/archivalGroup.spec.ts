  ...
  for (const useBagitLayout of [true, false]) {
    ...
    test(`can create an Archival Group from a Deposit, ${useBagitLayout?'':'NOT'} in Bagit layout`, async ({page, context}) => {
      ...
      await test.step('Add rights statement and access restrictions', async () => {
        ...
        await expect(archivalGroupPage.depositPage.objectsFolder).not.toContainText(archivalGroupPage.depositPage.selectedAccessConditions[0]);
        await expect(archivalGroupPage.depositPage.objectsFolder).not.toContainText(archivalGroupPage.depositPage.selectedAccessConditions[1]);
        await expect(archivalGroupPage.depositPage.objectsFolder).not.toContainText(archivalGroupPage.depositPage.selectedRightsOptionShortCode);
        ...
        await expect(archivalGroupPage.depositPage.objectsFolder).toContainText(archivalGroupPage.depositPage.selectedAccessConditions[0]);
        await expect(archivalGroupPage.depositPage.objectsFolder).toContainText(archivalGroupPage.depositPage.selectedAccessConditions[1]);
        await expect.soft(archivalGroupPage.depositPage.objectsFolder).toContainText(archivalGroupPage.depositPage.selectedRightsOptionShortCode);
        ...
        await expect(archivalGroupPage.depositPage.objectsFolder).toContainText(archivalGroupPage.depositPage.modifiedAccessConditions[0]);
        await expect(archivalGroupPage.depositPage.objectsFolder).toContainText(archivalGroupPage.depositPage.modifiedAccessConditions[1]);
        await expect(archivalGroupPage.depositPage.objectsFolder).toContainText(archivalGroupPage.depositPage.modifiedRightsOptionShortCode);
        ...
        await archivalGroupPage.depositPage.checkAccessRightsExist(metsXML, archivalGroupPage.depositPage.modifiedAccessConditions[0]);
        await archivalGroupPage.depositPage.checkAccessRightsExist(metsXML, archivalGroupPage.depositPage.modifiedAccessConditions[1]);
        await archivalGroupPage.depositPage.checkAccessRightsExist(metsXML, archivalGroupPage.depositPage.selectedAccessConditions[0], false);
        await archivalGroupPage.depositPage.checkAccessRightsExist(metsXML, archivalGroupPage.depositPage.selectedAccessConditions[1], false);
